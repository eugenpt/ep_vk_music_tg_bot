...
socket._GLOBAL_DEFAULT_TIMEOUT = 300
...
def albums_fun(message):
    ...
    if len(albums) == 0:
        ...
        bot.send_message(
            ...
            'Albums:',
            reply_markup=markup(
                [btn(album_name + '(%i)' % len(albums[album_name]), callback_data=['album', {'album_name': album_name}])
...
def album_fun(message, album_name=None):
    ...
    if len(album) == 0:
        ...
        home_fun(message, text='^ ^',
                 add_btns=[btn('Delete album', callback_data=['delete_album', {'album_name': album_name}])])
    ...
    #     add_ids_to_album_for_fun(message, ids=ids, album_name=list(albums.keys())[0])
        ...
        DEBUG['markup'] = markup([
                                     btn(album_name + ('(%i)' % len(albums[album_name]))
                                         , callback_data=encode_callback_data('/add_ids_to_album',
                                                                              {'ids': ids, 'album_name': album_name}))
                                     for album_name in albums
                                 ] + [
                                     btn('new album', callback_data='/new_album')
                                 ])
...
def add_ids_to_album_for_fun(message, *, ids, album_name):
...
def handle_docs_audio(message):
    ...
    if len(rec['matches']) == 0:
        ...
        get_page_search_fun(
            message,
            q=rec['track']['title'] + ' - ' + rec['track']['subtitle'],
        )
...
def message_handler(message):
    ...
    if get_chat_state(message.chat.id) == CONSTS.NEW_ALBUM:
        ...
        set_chat_state(message.chat.id, None)
        get_page_search_fun(message, q=message.text)
...
def callback_query(query):
    ...
    if query.data[0] == '/':
        ...
        logger.error('What the [%s] callback data??' % query.data)
        # ids = query.data
...
@bot.message_handler(commands=['get_page_search'])
def get_page_search_fun(message, page=0, edit=False, *, q):
    bot.send_chat_action(message.chat.id, "typing")
    global DEBUG
    DEBUG['message'] = message
    logger.info('Looking for ' + q)

    R = ep_vk_search(q, n_results_per_page=Options.n_results_per_page, page=page)
    ...
    if len(R) == 0:
        ...
        reply_markup = page_search_reply_markup(q, R, page)
...
def page_search_reply_markup(q, R, page):
    keyboard = [
        [btn(
            r['title_str'],
            callback_data=['get_vk_audio', {'ids': r['ids_string']}]
        )]
...
def page_button(q, page, delta_n):
    return btn(
        "<" if delta_n < 0 else ">",
        callback_data=['get_page_search', {'q': q, 'page': page + delta_n, 'edit': True}]
    )


# %% Audios

@bot.message_handler(commands=['get_vk_audio'])
def handle_vk_audio_by_ids(message, *, ids):
    chat_id = message.chat.id
    ...
    try:
        ...
        if 'telegram' in r:
            ...
            retry_sleep_s = 1
            tryj = 0
            while tryj < n_retries:
                tryj = tryj + 1
                logger.debug(f'sending audio by content try {tryj}')
                ...
                try:
                    ...
                    logger.error(f'upload error? {task.result}')
                    sleep(retry_sleep_s)
                    continue
